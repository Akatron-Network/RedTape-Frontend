  ...
  return (
      <div className='xl:col-span-3 col-span-6 grid gap-[1px]'>
        ...
        <div className={tasks_editable ? 'w-full flex flex-row shadow-input ellipsis pointer-events-none rounded-md' : 'w-full flex flex-row shadow-input ellipsis rounded-md'}>
          <span className="w-1/3 min-w-[120px] truncate flex justify-center min-h-[34px] border border-alica_blue_light text-sm items-center bg-steel_blue_light font-medium text-prussian_blue px-1 py-[6px] rounded-l-md">Görev</span>
          <input type="text" ref={(el) => {if (tasksNameRef.current !== null) tasksNameRef.current[props.alias] = el}} onBlur={(e) => {task_steps[props.alias].name = e.target.value}} className="w-2/3 min-h-[34px] py-[6px] bg-white border border-white text-prussian_blue text-sm placeholder:text-mn_blue placeholder:opacity-70 rounded-r-md focus:border-shadow_blue focus:ring-transparent block" placeholder="Görev girin" required />
        </div>
        ...
        <div className={tasks_editable ? 'w-full flex flex-row shadow-input pointer-events-none rounded-md' : 'w-full flex flex-row shadow-input rounded-md'}>
          <span className="w-1/3 min-w-[120px] truncate flex min-h-[34px] justify-center items-center border border-alica_blue_light px-1 py-[6px] text-sm bg-steel_blue_light rounded-l-md text-prussian_blue">Görevli</span>
          <select ref={(el) => {if (tasksResponsibleUsernameRef.current !== null) tasksResponsibleUsernameRef.current[props.alias] = el}} defaultValue="default" onChange={(e) => {task_steps[props.alias].responsible_username = e.target.value}} required className="block w-2/3 min-h-[34px] py-[6px] text-sm text-prussian_blue border border-white bg-white focus:ring-transparent focus:border-shadow_blue rounded-r-md">
        ...
        <div className={tasks_editable ? 'w-full flex flex-row shadow-input pointer-events-none rounded-md' : 'w-full flex flex-row shadow-input rounded-md'}>
          <span className="w-1/3 min-w-[120px] truncate flex justify-center min-h-[34px] border border-alica_blue_light text-sm items-center bg-steel_blue_light font-medium text-prussian_blue px-1 py-[6px] rounded-l-md">Teslim Tarihi</span>
          {!tasks_editable ? 
            <input type="date" defaultValue={new Date().toISOString().split('T')[0]} ref={(el) => {if (tasksPlannedFinishDate.current !== null) tasksPlannedFinishDate.current[props.alias] = el}} onBlur={(e) => {task_steps[props.alias].planned_finish_date = e.target.value}} className="w-2/3 min-h-[34px] py-[6px] bg-white border border-white text-prussian_blue text-sm placeholder:text-mn_blue placeholder:opacity-70 rounded-r-md focus:border-shadow_blue focus:ring-transparent block" required />
            : 
