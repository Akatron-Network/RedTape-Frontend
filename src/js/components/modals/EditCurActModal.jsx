        ...
        <div className="relative w-full h-full max-w-lg md:h-auto">
          <div className="relative bg-ghost_white rounded-sm shadow">
            ...
            <div className="pt-4 px-5 grid grid-cols-1 gap-[1px]">
              <div className='col-span-1'><InputDate name={"Tarih"} reference={cur_act_data.curActDateRef} defaultValue={cur_act_data.date.current} /></div>
              <div className='col-span-1'><InputDefault name={"Açıklama"} reference={cur_act_data.curActDescriptionRef} /></div>
              <div className='col-span-1'><InputDate name={"Vade Tarihi"} reference={cur_act_data.curActExpiryDateRef} /></div>
              <div className='col-span-1'><InputSelect name={"Borç-Alacak"} reference={cur_act_data.curActDebtAmountRef} options={["Borç", "Alacak"]} func={() => {}} /></div>
              <div className='col-span-1'><InputDefault name={"Tutar"} reference={cur_act_data.curActBalanceRef} /></div>
            </div>
            <div className="flex items-center px-5 py-3 space-x-2 border-t border-steel_blue_light rounded-b justify-end">
              ...
              <button type="button" className="save-btn ml-2 float-right" onClick={() => cur_act_data.editCurrent(cur_act_data.current_details.id)}><i className="fa-solid fa-floppy-disk mr-2"></i>Kaydet</button> 
            </div>
          </div>
        </div>
      </div>
